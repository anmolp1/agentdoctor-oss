...
describe("False positive regression", () => {
  it.each(healthyFixtures)(
    "no critical findings on %s beyond known baseline",
    async (fixture) => {
      ...
      const criticals = result.diagnostics.findings.filter(
  ...
  it.each(healthyFixtures)(
    "no warning findings on %s beyond known baseline",
    async (fixture) => {
      const result = await analyze({
        logFiles: [path.join(FIXTURES_DIR, fixture)],
        outputFormat: "json",
      ...
      const warnings = result.diagnostics.findings.filter(
