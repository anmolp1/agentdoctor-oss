...
export function computeInstructionCoherence(
  ...
  const phantomFindings = driftFindings.filter(
    (f) => f.title.toLowerCase().includes("phantom"),
  );
  const orphanFindings = driftFindings.filter(
    (f) => f.title.toLowerCase().includes("orphan"),
  );
  ...
  const contradictionFindings = driftFindings.filter(
    (f) => f.title.toLowerCase().includes("contradict"),
  );
  ...
  if (orphanFindings.length > 0)
    flags.push(`${orphanFindings.length} orphaned tool(s) found`);
  if (contradictionFindings.length > 0)
    flags.push("Potentially contradictory directives found");
